...
function statusTone(status: string | undefined): QueueTone {
  ...
  if (/(running|pending|ready|draft|warning|waiting)/.test(value))
    return "warning";
...
function runHref(run: WorkbenchRunSummary): string {
  ...
  if (!isKnownTaskRun)
    return `/workbench?task_run_id=${encodeURIComponent(run.task_run_id)}`;
...
function buildQueue(
  ...
  tasks
    .filter((task) => {
      ...
      return (
        run &&
        statusTone(`${run.execution_status} ${run.delivery_status}`) ===
          "danger"
      );
    })
    .slice(0, 3)
    .forEach((task) => {
      ...
      items.push({
        ...
        meta: `${task.workspace_name || "未绑定项目"} · ${task.workflow_name || "未绑定工作流"}`,
...
export default function WorkbenchPage() {
  ...
  const indexedWorkspaces = workspaces.filter(
    (workspace) => workspace.indexed === 1,
  ).length;
  const failedWorkspaces = workspaces.filter(
    (workspace) => workspace.indexed === -1,
  ).length;
  const reportCount = workspaces.reduce(
    (total, workspace) => total + workspace.reports.length,
    0,
  );
  const activeRuns = runs.filter(
    (run) => statusTone(run.execution_status) === "warning",
  ).length;
  const queueItems = useMemo(
    () => buildQueue(workspaces, tasks, runs),
    [runs, tasks, workspaces],
  );
  const currentRun =
    runs[0] ?? tasks.find((task) => task.latest_run)?.latest_run ?? null;
    ...
    <div className="ct-home-shell ct-home-workbench w-full px-4 pb-4 xl:px-6">
      ...
      >
        <div className="ct-home-metric rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
          ...
          <div className="ct-home-metric-value font-display text-2xl font-bold text-amber-700">
            {isInitialLoading ? "--" : queueItems.length}
          </div>
        </div>
        <div className="ct-home-metric rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
          <div className="mb-3 flex items-center gap-2 text-xs font-medium text-on-surface-variant">
          ...
          <div className="ct-home-metric-value font-display text-2xl font-bold text-green-700">
            {isInitialLoading ? "--" : activeRuns}
          </div>
        </div>
        <div className="ct-home-metric rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
          <div className="mb-3 flex items-center gap-2 text-xs font-medium text-on-surface-variant">
          ...
          <div className="ct-home-metric-value font-display text-2xl font-bold text-primary">
            {isInitialLoading
              ? "--"
              : `${indexedWorkspaces}/${workspaces.length}`}
          </div>
        </div>
        <div className="ct-home-metric rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
          <div className="mb-3 flex items-center gap-2 text-xs font-medium text-on-surface-variant">
          ...
          <div className="ct-home-metric-value font-display text-2xl font-bold text-on-surface">
            {isInitialLoading ? "--" : reportCount}
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,0.8fr)]">
        <section
          aria-label="待处理队列"
          className="ct-home-panel rounded-lg border border-outline-variant/30 bg-surface-container-low p-4"
        >
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link
              href="/tasks"
              className="inline-flex items-center gap-1.5 rounded-lg border border-outline-variant/40 bg-surface px-3 py-2 text-sm font-medium text-on-surface hover:bg-surface-container"
            >
              打开任务中心
              <ArrowRight size={14} />
            </Link>
            ...
            <div className="space-y-2">
              {queueItems.map((item) => (
                <Link
                  key={item.id}
                  href={item.href}
                  className="ct-home-queue-row grid gap-3 rounded-lg border border-outline-variant/30 bg-surface px-3 py-2.5 hover:bg-surface-container sm:grid-cols-[minmax(0,1fr)_auto_auto]"
                >
                  <div className="min-w-0">
                    <div
                  ...
                  <span
                    className={`inline-flex w-fit items-center rounded-full border px-2.5 py-1 text-xs font-semibold ${toneClasses(item.tone)}`}
                  >
                    {item.status}
                  </span>
          ...
          >
            ...
            {currentRun ? (
              <div className="rounded-lg border border-outline-variant/30 bg-surface p-4">
                <span
                  className={`inline-flex rounded-full border px-2.5 py-1 text-xs font-semibold ${toneClasses(statusTone(`${currentRun.execution_status} ${currentRun.delivery_status}`))}`}
                >
                  {statusLabel(
                    currentRun.execution_status || currentRun.delivery_status,
                  )}
                </span>
                <p className="mt-3 truncate text-sm font-semibold text-on-surface">
                  {compactMachineToken(
                    currentRun.workflow_id || currentRun.task_id,
                    28,
                  )}
                </p>
                <p className="mt-1 text-xs text-on-surface-variant">
                  ...
                  · attempt {currentRun.attempt_number}
                </p>
                <Link
                  href={runHref(currentRun)}
                  className="mt-4 inline-flex items-center gap-1.5 rounded-lg bg-primary px-3 py-2 text-sm font-semibold text-on-primary"
                >
                  进入复盘
                  <ArrowRight size={14} />
                </Link>
          ...
          >
            ...
            <div className="space-y-2">
              {workspaces.slice(0, 4).map((workspace) => {
                ...
                return (
                  <Link
                    key={workspace.id}
                    href={`/workspaces/${workspace.id}`}
                    className="flex items-center justify-between gap-3 rounded-lg border border-outline-variant/30 bg-surface px-3 py-2 hover:bg-surface-container"
                  >
                    <span className="min-w-0">
                      ...
                      <span className="block truncate text-xs text-on-surface-variant">
                        {hasPath ? workspace.repo_path : "未绑定本地文件夹"}
                      </span>
                    </span>
                    <span
                      className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-semibold ${toneClasses(tone)}`}
                    >
                      {label}
                    </span>
                  </Link>
              ...
              {!isInitialLoading && workspaces.length === 0 && (
                <Link
                  href="/workspaces/new"
                  className="flex items-center justify-center gap-2 rounded-lg border border-dashed border-outline-variant/50 bg-surface px-3 py-6 text-sm font-semibold text-primary"
                >
                  <Plus size={16} />
                  创建项目基底
                </Link>
      ...
      <section
        aria-label="证据与知识"
        className="ct-home-panel mt-4 rounded-lg border border-outline-variant/30 bg-surface-container-low p-4"
      >
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="flex items-center gap-2 text-base font-semibold text-on-surface">
        ...
        <div className="grid gap-3 md:grid-cols-3">
          ...
          <div className="rounded-lg border border-outline-variant/30 bg-surface p-4">
            ...
            <p className="mt-2 font-display text-xl font-bold text-red-600">
              {isInitialLoading ? "--" : failedWorkspaces}
            </p>
          </div>
          <div className="rounded-lg border border-outline-variant/30 bg-surface p-4">
            <p className="text-xs font-medium text-on-surface-variant">
