...
export function WorkflowWizard() {
  ...
  return <main className="ct-v2-wizard" data-testid="workflow-wizard-ready" data-hydrated={hydrated ? "true" : "false"}>
    <header className="ct-v2-wizard-header"><Link href="/workflows"><ArrowLeft size={16} />工作流库</Link><div><p>{workflowParam ? "V2 兼容编辑" : "兼容模式新建"}</p><h1>{name || "未命名工作流"}</h1></div>{workflowParam && versionParam ? <button type="button" onClick={async () => { setBusy(true); setError(""); try { const copied = await workflowsApi.copyVersionToV3(workflowParam, versionParam); router.push(copied.designer_url); } catch (cause) { setError(cause instanceof Error ? cause.message : "复制为 V3 失败"); } finally { setBusy(false); } }} disabled={busy}><Copy size={14} />复制为 V3 画布</button> : <span>{workflowId ? `草稿 · ${workflowId}` : "尚未创建草稿"}</span>}</header>
