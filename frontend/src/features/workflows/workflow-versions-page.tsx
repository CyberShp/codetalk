...
export function WorkflowVersionsPage({ workflowId }: { workflowId: string }) {
  ...
  return <main className="ct-v2-library">
    <header className="ct-v2-page-header"><div><Link className="ct-v2-back-link" href="/workflows"><ArrowLeft size={15} />工作流库</Link><h1>{detail?.presentation?.label ?? detail?.name ?? "工作流"} · 版本</h1><span>{migrationOnly ? "历史 V1/V2 版本保持只读；迁移前需先查看并确认 V3 预览。" : "发布版本不可修改；继续改动时从已发布版本创建新草稿。"}</span></div>{detail?.v2 && !detail.v2.current_draft_version_id && detail.v2.status === "active" && <button type="button" className="ct-v2-primary-button" onClick={() => void createDraft()}><Plus size={15} />{migrationOnly ? "查看 V3 迁移预览" : "创建新草稿"}</button>}</header>
