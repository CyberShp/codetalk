...
export function SemanticLibraryPage() {
  ...
  return <main className="ct-asset-page">
    <header className="ct-v2-page-header"><div><span className="ct-v2-eyebrow">测试知识资产</span><h1>语义用例库</h1><p>管理可复用的测试场景。当前检索使用 FTS5 关键字匹配，不是向量搜索。</p></div><button className="ct-v2-primary-button" type="button" onClick={() => setImporting(true)}><FileUp size={16} />导入用例</button></header>
...
function TagList({ items }: { items: string[] }) { return <div className="ct-asset-tags">{items.slice(0, 3).map((item) => <span key={item}>{item}</span>)}{items.length > 3 && <span>+{items.length - 3}</span>}</div>; }
