...
function TaskReview({ task,name,workspace,definition,inputs,executionOverrides,outputOverrides,onFinish,busy }:{task:WorkbenchTask|null;name:string;workspace:string;definition:Definition;inputs:Record<string,unknown>;executionOverrides:Record<string,unknown>;outputOverrides:Record<string,unknown>;onFinish:(m:"draft"|"ready"|"run")=>Promise<void>;busy:boolean}) { const enabledOutputs=(definition.outputs||[]).filter((item)=>outputEnabled(item,outputOverrides)).length; return <div className="ct-v2-task-step"><h2>检查并运行</h2><div className="ct-v2-task-review"><section><strong>{name}</strong><span>{workspace}</span><small>{task?.workflow_name} · {task?.workflow_version_id}</small></section><section><dl><div><dt>输入</dt><dd>{Object.keys(inputs).length}/{(definition.inputs||[]).filter((item)=>!isWorkspaceInputDefinition(item)).length}</dd></div><div><dt>执行节点</dt><dd>{(definition.steps||[]).length}</dd></div><div><dt>任务覆盖</dt><dd>{Object.keys((executionOverrides.nodes||{}) as object).length}</dd></div><div><dt>输出</dt><dd>{enabledOutputs+(((outputOverrides.custom_outputs||[]) as unknown[]).length)}</dd></div></dl></section></div><div className="ct-v2-finish-actions"><button type="button" disabled={busy} onClick={()=>void onFinish("draft")}>保存草稿</button><button type="button" disabled={busy} onClick={()=>void onFinish("ready")}>保存为就绪任务</button><button className="ct-v2-primary-button" type="button" disabled={busy} onClick={()=>void onFinish("run")}>{busy&&<Loader2 className="animate-spin" size={14}/>}保存并运行</button></div></div>; }
