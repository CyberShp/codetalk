...
export function WorkbenchTaskDetailPage({ taskId }: { taskId: string }) {
  ...
  const run = async () => { setRunning(true); setError(""); try { const attempt = await workbenchTasksApi.createRun(taskId); await api.workbench.taskRuns.execute(attempt.task_run_id, 0, true); await load(); setTab("运行记录"); } catch (cause) { setError(cause instanceof Error ? cause.message : "运行启动失败"); } finally { setRunning(false); } };
  ...
  return <main className="ct-v2-task-detail">
    ...
    <section className="ct-v2-task-detail-body">
      {tab === "概览" && <div className="ct-v2-overview-grid"><section><h2>任务定义</h2><DefinitionRow label="工作流" value={task.workflow_name} detail={task.workflow_version_id} /><DefinitionRow label="工作空间" value={task.workspace_name} /><DefinitionRow label="标签" value={task.tags.join("、") || "—"} /></section><section><h2>最近运行</h2>{task.latest_run ? <><DefinitionRow label="Attempt" value={String(task.latest_run.attempt_number)} /><DefinitionRow label="执行状态" value={taskStatusLabel(taskExecutionLabels, task.latest_run.execution_status)} /><DefinitionRow label="质量 / 交付" value={`${taskStatusLabel(taskQualityLabels, task.latest_run.quality_status)} / ${taskStatusLabel(taskDeliveryLabels, task.latest_run.delivery_status)}`} /><Link href={`/workbench?task_run_id=${task.latest_run.task_run_id}`}>打开运行驾驶舱</Link></> : <p>还没有运行记录。</p>}</section></div>}
      {tab === "运行记录" && <div className="ct-v2-attempt-list">{runs.map((item) => <article key={item.task_run_id}><span>Attempt {item.attempt_number}</span><strong>{taskStatusLabel(taskExecutionLabels, item.execution_status)}</strong><small>{formatDate(item.created_at)}</small>{item.parent_task_run_id && <em><RotateCcw size={13} />来自 {item.parent_task_run_id.slice(0, 18)}</em>}<Link href={`/workbench?task_run_id=${item.task_run_id}`}>查看运行</Link></article>)}{!runs.length && <p>还没有运行记录。</p>}</div>}
