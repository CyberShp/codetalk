...
import { Archive, ArrowLeft, Loader2, MessageSquareText, Play, RotateCcw } from "lucide-react";
...
export function WorkbenchTaskDetailPage({ taskId }: { taskId: string }) {
  ...
  return <main className="ct-v2-task-detail">
    ...
    <section className="ct-v2-task-detail-body">
      {tab === "概览" && <div className="ct-v2-overview-grid"><section><h2>任务定义</h2><DefinitionRow label="工作流" value={task.workflow_name} detail={task.workflow_version_id} /><DefinitionRow label="工作空间" value={task.workspace_name} /><DefinitionRow label="标签" value={task.tags.join("、") || "—"} /></section><section><h2>最近运行</h2>{task.latest_run ? <><DefinitionRow label="Attempt" value={String(task.latest_run.attempt_number)} /><DefinitionRow label="执行状态" value={taskStatusLabel(taskExecutionLabels, task.latest_run.execution_status)} />{isV3RunSummary(task, task.latest_run) ? <><DefinitionRow label="产物校验" value={taskStatusLabel(taskArtifactValidationLabels, task.latest_run.artifact_validation_status || "not_started")} /><DefinitionRow label="专业治理" value={taskStatusLabel(taskGovernanceLabels, task.latest_run.governance_status || "not_requested")} /><DefinitionRow label="交付状态" value={taskStatusLabel(taskDeliveryLabels, task.latest_run.delivery_status)} /></> : <DefinitionRow label="质量 / 交付" value={`${taskStatusLabel(taskQualityLabels, task.latest_run.quality_status)} / ${taskStatusLabel(taskDeliveryLabels, task.latest_run.delivery_status)}`} />}<Link href={`/tasks/${taskId}/runs/${task.latest_run.task_run_id}`}>打开运行驾驶舱</Link></> : <p>还没有运行记录。</p>}</section>{Boolean(task.ai_origins?.length) && <section className="ct-v2-task-ai-links"><h2>关联 AI 线程</h2><p>任务来源和围绕运行的后续分析都保留在这里。</p>{task.ai_origins?.map((origin) => <div key={`${origin.relation_type}:${origin.conversation_id}:${origin.task_run_id || origin.ai_run_id}`}><MessageSquareText size={15} /><span><strong>{origin.relation_type === "task_created_from_ai" ? "任务来源" : "运行分析"}</strong><small>{origin.task_run_id ? `关联 Run ${origin.task_run_id.slice(0, 12)}` : "由 AI 线程创建"}</small></span><Link href={`/ai/${encodeURIComponent(origin.conversation_id)}`}>打开线程</Link></div>)}</section>}</div>}
      ...
      {tab === "执行配置" && <div className="ct-v2-definition-sections"><section><h2>继承基线</h2><p>系统默认 → {task.workflow_name} → 当前任务覆盖</p><DefinitionRow label="执行档位" value={task.execution_profile_id || "工作流默认档位"} /></section><KeyValueRows values={task.execution_overrides} empty="未设置任务级覆盖，使用工作流版本默认配置。" /></div>}
...
function isV3RunSummary(task: WorkbenchTask, run: WorkbenchTask["latest_run"]): run is NonNullable<WorkbenchTask["latest_run"]> & { artifact_validation_status: string; governance_status: string } { return Boolean(run && (run.compiled_contract_version === 3 || task.workflow_version?.compiled_definition?.compiled_contract_version === 3)); }
