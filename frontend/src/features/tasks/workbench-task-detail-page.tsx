...
export function WorkbenchTaskDetailPage({ taskId }: { taskId: string }) {
  ...
  return <main className="ct-v2-task-detail">
    ...
    <section className="ct-v2-task-detail-body">
      ...
      {tab === "输出" && <div className="ct-v2-definition-sections"><section><h2>交付件档案</h2><p>档案在启动运行时解析并冻结，工作流只声明需要哪些输出。</p><DefinitionRow label="当前选择" value={artifactProfiles.find((item) => item.id === artifactProfileId)?.name || "按工作空间、特性标签和本机默认规则选择"} /><Link href="/artifact-profiles">管理交付件档案</Link></section><KeyValueRows values={task.output_overrides} empty="未设置任务级输出覆盖，使用工作流版本输出契约。" /></div>}
