...
export function TaskCenterPage() {
  ...
  return <main className="ct-v2-library ct-v2-task-center">
    ...
    <section className="ct-v2-task-filters" aria-label="任务筛选">
      ...
      <label><span>工作流</span><select ref={bindFilterSelect} data-filter-key="workflow_id" key={`workflow-${query.workflow_id}`} defaultValue={query.workflow_id}><option value="">全部</option>{workflows.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
