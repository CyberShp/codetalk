...
export function TaskCenterPage() {
  ...
  return <main className="ct-v2-library ct-v2-task-center">
    ...
    <section className="ct-v2-task-filters" aria-label="任务筛选">
      ...
      <label><span>工作流</span><select ref={bindFilterSelect} data-filter-key="workflow_id" key={`workflow-${query.workflow_id}`} defaultValue={query.workflow_id}><option value="">全部</option>{workflows.map((item) => <option key={item.id} value={item.id}>{item.presentation?.label ?? item.name}</option>)}</select></label>
    ...
    <div className="ct-v2-table-shell"><table className="ct-v2-table"><thead><tr><th>任务</th><th>生命周期</th><th>当前运行</th><th>工作流 / 版本</th><th>工作空间</th><th>质量</th><th>交付</th><th>更新时间</th><th aria-label="操作" /></tr></thead><tbody>{tasks.map((task) => <tr key={task.task_id}><td><Link href={`/tasks/${task.task_id}`}><strong>{task.name}</strong><small>{task.description || task.tags.join(" · ") || "无描述"}</small></Link></td><td><Status value={task.lifecycle_status} label={taskStatusLabel(taskLifecycleLabels, task.lifecycle_status)} /></td><td><Status value={task.latest_run?.execution_status || "not_started"} label={taskStatusLabel(taskExecutionLabels, task.latest_run?.execution_status || "not_started")} /><small>{task.latest_run ? `Attempt ${task.latest_run.attempt_number}` : "—"}</small></td><td><strong>{task.workflow_name}</strong><small>{task.workflow_version_number ? `V${task.workflow_version_number}` : "历史版本"}</small></td><td><span className="ct-v2-cell-text" title={task.workspace_name}>{task.workspace_name}</span></td><td>{taskStatusLabel(taskQualityLabels, task.latest_run?.quality_status || "not_checked")}</td><td>{taskStatusLabel(taskDeliveryLabels, task.latest_run?.delivery_status || "none")}</td><td>{formatTime(task.updated_at)}</td><td><div className="ct-v2-row-actions"><Link href={`/tasks/${task.task_id}`} title="打开任务"><ExternalLink size={15} /></Link><button type="button" title="复制任务" onClick={() => void cloneTask(task.task_id)}><Copy size={15} /></button>{task.lifecycle_status !== "archived" && <button type="button" title="归档任务" onClick={() => void archiveTask(task.task_id)}><Archive size={15} /></button>}</div></td></tr>)}{!loading && !tasks.length && <tr><td colSpan={9}><div className="ct-v2-table-empty">没有符合当前筛选条件的任务</div></td></tr>}</tbody></table></div>
    <Pagination page={query.page} total={total} onPage={(page) => setFilter("page", page === 1 ? "" : String(page))} />
  </main>;
}

function HistoricalRuns({ runs }: { runs: WorkbenchRunSummary[] }) { return <section className="ct-v2-history-band"><div><h2>历史运行</h2></div><div>{runs.length ? runs.slice(0, 12).map((run) => <Link key={run.task_run_id} href={`/workbench?task_run_id=${run.task_run_id}`}><span>历史运行记录</span><strong>{taskStatusLabel(taskExecutionLabels, run.execution_status)}</strong><small>{formatTime(run.created_at)}</small></Link>) : <span>没有旧运行</span>}</div></section>; }
