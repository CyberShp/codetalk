...
} from "@/features/runs/run-event-semantics";

const tabs = ["运行概览", "实时输出", "工具调用", "事件日志"] as const;
...
export function RunCockpitPage({ taskId, runId }: { taskId: string; runId: string }) {
  ...
  const [tab, setTab] = useState<(typeof tabs)[number]>("运行概览");
  ...
  const visibleEvents = useMemo(() => {
    ...
    return eventSource.filter((item) => {
      ...
      if (tab === "实时输出" && !isMeaningfulLiveEvent(item)) return false;
      if (tab === "工具调用" && !isStructuredToolEvent(item)) return false;
  ...
  return <main className="ct-v2-run-cockpit">
    ...
    <section className="ct-v2-run-workspace">
      <div className="ct-v2-run-main">
        ...
        {tab === "运行概览" ? <RunSummary summary={summary} events={events} artifactCount={publicArtifacts.length} nodeLabels={nodeLabels} failed={failed} partial={partial} selectedNodeId={inspectedNode?.id || ""} onSelectNode={setSelectedNodeId} onRetry={() => void retry()} actionBusy={actionBusy} /> : <>
          ...
          <div className="ct-v2-event-viewport" ref={eventViewport} onScroll={(event) => { const target = event.currentTarget; if (target.scrollHeight - target.scrollTop - target.clientHeight > 36) setAutoScroll(false); }}>
            ...
            {visibleEvents.length ? (tab === "工具调用" ? pairedToolCalls(visibleEvents).map((item) => <ToolCallRow key={item.id} item={item} nodeLabels={nodeLabels} />) : tab === "实时输出" ? visibleEvents.map((item) => <LiveOutputRow key={item.event_id} item={item} nodeLabels={nodeLabels} />) : visibleEvents.map((item) => <EventRow key={item.event_id} item={item} nodeLabels={nodeLabels} />)) : <div className="ct-v2-event-empty">{tab === "实时输出" ? "执行器尚未公开可读的实时文本。" : tab === "工具调用" ? "执行器未公开带名称和调用编号的结构化工具记录；不会用 session 通知冒充工具调用。" : "暂无符合条件的公开事件。"}</div>}
          </div>
        </>}
      </div>
...
function RunSummary({ summary, events, artifactCount, nodeLabels, failed, partial, selectedNodeId, onSelectNode, onRetry, actionBusy }: { summary: PreparedWorkbenchTaskRun["run_ui_summary"]; events: WorkbenchTaskRunEvent[]; artifactCount: number; nodeLabels: NodeLabelMap; failed: boolean; partial: boolean; selectedNodeId: string; onSelectNode: (id: string) => void; onRetry: () => void; actionBusy: boolean }) { const recoveredNodes = summary?.nodes.filter((node) => node.recovered_from_partial) || []; const recovered = recoveredNodes.length > 0; const recoveredLabel = recoveredNodes.map(publicNodeLabel).join("、"); const nodes = summary?.nodes || []; const completedCount = nodes.filter((node) => ["completed", "success"].includes(node.status || "")).length; const currentActivity = selectOverviewActivity(events); return <div className="ct-v2-run-summary"><section><h2>{failed ? "运行在节点处停止" : partial ? "运行保留了部分结果" : "节点进度"}</h2><div className="ct-v2-run-overview-metrics"><article><span>已完成节点</span><strong>{completedCount} / {nodes.length}</strong></article><article><span>当前节点</span><strong>{summary?.current_node ? publicNodeLabel(summary.current_node) : "等待调度"}</strong></article><article><span>已生成文件</span><strong>{artifactCount}</strong></article></div><div className="ct-v2-node-timeline">{nodes.map((node) => { const nodeName = publicNodeLabel(node); return <button type="button" key={node.id} className={`is-${node.status || "prepared"} ${selectedNodeId === node.id ? "is-selected" : ""}`} onClick={() => onSelectNode(node.id)} aria-label={`查看节点 ${nodeName}`}><span>{node.status === "completed" || node.status === "success" ? <CheckCircle2 size={15} /> : node.status === "running" ? <Loader2 className="animate-spin" size={15} /> : node.status === "failed" || node.status === "error" || node.status === "interrupted" ? <AlertTriangle size={15} /> : <i />}</span><strong>{nodeName}</strong><small>{node.status_label}</small></button>; })}</div></section><StageProgressPanel events={events} stageProgress={summary?.test_activity_stage_progress} runPartial={partial} recovered={recovered} recoveredLabel={recoveredLabel} onRetry={onRetry} busy={actionBusy} /><section className="ct-v2-current-activity"><h2>当前进展</h2>{currentActivity ? <><header><span>{eventNodeLabel(currentActivity, nodeLabels)}</span><time>{new Date(currentActivity.created_at).toLocaleTimeString("zh-CN", { hour12: false })}</time></header><p>{publicNodeText(eventText(currentActivity), nodeLabels) || eventMessage(currentActivity, nodeLabels)}</p></> : <p>正在等待执行器公开进展。</p>}</section></div>; }
...
function LiveOutputRow({ item, nodeLabels }: { item: WorkbenchTaskRunEvent; nodeLabels: NodeLabelMap }) { return <article className={`ct-v2-live-output is-${item.event_kind}`}><header><span>{eventNodeLabel(item, nodeLabels)}</span><time>{new Date(item.created_at).toLocaleTimeString("zh-CN", { hour12: false })}</time></header><pre>{publicNodeText(eventText(item), nodeLabels)}</pre></article>; }
...
function ToolCallRow({ item, nodeLabels }: { item: PairedToolCall; nodeLabels: NodeLabelMap }) { const source = item.use || item.result; if (!source) return null; const tool = String(source.payload.tool || source.payload.name || source.payload.tool_id || "工具"); const inputSummary = item.use ? eventDetail(item.use, nodeLabels) : ""; const resultSummary = item.result ? eventDetail(item.result, nodeLabels) || eventMessage(item.result, nodeLabels) : "等待工具返回结果"; return <article className={`ct-v2-tool-call ${item.result ? "is-complete" : "is-running"}`}><header><time>{new Date(source.created_at).toLocaleTimeString("zh-CN", { hour12: false })}</time><strong>{tool}</strong><span>{item.result ? "已完成" : "调用中"}</span></header><p>{inputSummary ? `输入：${inputSummary}` : item.use ? eventMessage(item.use, nodeLabels) : "收到工具结果"}</p><pre>{resultSummary}</pre></article>; }
...
function eventDetail(item: WorkbenchTaskRunEvent, nodeLabels: NodeLabelMap = new Map()) { const value = item.payload.delta ?? item.payload.text ?? item.payload.content ?? item.payload.output ?? item.payload.error ?? item.payload.detail ?? item.payload.input ?? item.payload.arguments ?? ""; const text = typeof value === "string" ? value : value ? JSON.stringify(value, null, 2) : ""; return publicNodeText(text, nodeLabels); }
