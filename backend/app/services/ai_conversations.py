...
def _codex_style_answer_instruction() -> str:
    return (
        ...
        "- 风险、原因、修改点、验证方式分开写。\n"
        "- 对比“已验证事实”和“待验证预期”时使用小标题和 bullet，不要输出 Markdown 表格。\n"
...
def _build_prompt(
    ...
    *,
    ...
    system = (
        ...
        "不要声称读过未出现在引用里的文件。\n"
        "源码行只能证明其字面可见的条件、状态更新或调用；不得据此直接断言 TCP 状态、"
        "资源已回收、协议响应已发送或端到端结果已发生。回答外部可观测行为时，必须分成"
        "“已验证源码事实”和“待通过黑盒操作验证的预期”，缺少直接证据时明确标记为待验证。\n"
