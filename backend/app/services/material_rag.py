...
async def _get_active_embedding_model_id() -> str | None:
    """Return the active embedding model config ID, or None."""
...
async def _get_embedding_client() -> tuple | None:
    """Create an embedding client from the active embedding model config.

    Returns (client, config_id) tuple, or None if no embedding model is configured.
    """
    try:
...
async def embed_material(material_id: str, workspace_id: str) -> int:
    ...
    result = await _get_embedding_client()
    if result is None:
    ...
    async with aiosqlite.connect(settings.sqlite_db) as db:
        ...
        for i, (chunk_content, embedding) in enumerate(zip(chunks, embeddings)):
            await db.execute(
                ...
                "(id, material_id, workspace_id, embedding_model_id, chunk_index, content, embedding, token_count, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    str(uuid.uuid4()),
                    material_id,
...
async def retrieve_chunks(
    ...
    Returns list of {content, filename, material_id, score} dicts, sorted by relevance.
    Returns empty list if no embedding model is configured or no chunks exist.
    Only returns chunks generated by the currently active embedding model.
    """
    result = await _get_embedding_client()
    if result is None:
        return []
    ...
    async with aiosqlite.connect(settings.sqlite_db) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
            "SELECT mc.content, mc.embedding, mc.material_id, wm.filename "
    ...
    for row in rows:
        ...
        scored.append((score, row["content"], row["filename"], row["material_id"]))
    ...
    return [
        ...
        for score, content, filename, mat_id in scored[:top_k]
    ]
