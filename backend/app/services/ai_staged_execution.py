    ...
    *,
    ...
    if "iscsi" in token_set and "login" in token_set:
        required_protocol_anchor_terms = {
            ...
            # This is where the first Login payload transitions out of the
            # connection-start timeout regime.  It must be present whenever
            # the workflow asks for timeout, recovery, or resource behavior;
            # otherwise later SFMEA repair cannot make a source-verifiable
...
def _stage_format_rules(stage_id: str, artifact: str) -> list[str]:
    ...
    rules = {
        ...
        "black_box_cases": (
            ...
            "- 若质量反馈提供 coverage_targets，每条针对这些目标的用例必须在 coverage_target_ids 中"
            "逐字列出它实际覆盖的 FLOW-COND/FLOW-STATE/RESOURCE 标识；source_or_test_evidence "
            "仍只保留真实源码或测试锚点。整个输出数组必须让 quality_retry_feedback 中每一个 "
            "coverage_targets.id 至少出现一次：先把目标分配给已有 case_id，只有没有合适的外部测试"
            "场景时才新增原子用例。不得只新增少量用例后保留未绑定的旧用例；也不得把未在本用例"
            "外部步骤、预期结果和观测点中验证的目标塞入 coverage_target_ids，不能用一个空泛用例"
            "覆盖所有目标。\n"
