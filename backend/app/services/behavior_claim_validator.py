...
def build_behavior_claim_audit_prompt(request: dict[str, Any]) -> str:
    return "\n".join(
        [
            ...
            "若 sfmea_row_behavior 的 statement.risk_status 是 test_hypothesis，它不是已观测缺陷；但 failure_mode、cause、effect、mechanism、evidence_interpretation 中任何关于当前实现的陈述仍必须逐项受源码支持。源码表明相反行为时判 contradicts，并用 field_patch 把该行改成准确、待验证的测试假设，或删除该风险。不得因为文本写了“待验证”就只核对锚点存在。",
            ...
            "若 black_box_case_behavior 的 statement.case_type 是 black_box_hypothesis，不得要求源码已经实现该测试的测量点或 harness；但 expected_result、observability、failure_diagnostics 中的协议状态、日志、连接状态、超时或资源行为仍必须与源码一致。外部测试工具可以是假设，产品行为不可以是假设性放行。",
