...
def test_agent_json_validates_component_source_files(tmp_path):
    from app.services.external_agent_discovery import parse_agent_output
    ...
    (src / "PaymentDocs.mdx").write_text("export const amount = 1\n\n# Payment\n", encoding="utf-8")
    raw = json.dumps({
        "candidate_files": [
            ...
            {"path": "src/components/PaymentPage.astro", "reason": "Astro source", "confidence": "high"},
            {"path": "src/components/PaymentDocs.mdx", "reason": "MDX source", "confidence": "high"},
        ],
        "candidate_entries": [],
    })
...
def test_agent_output_aggregates_responses_output_fragments(tmp_path):
    from app.services.external_agent_discovery import parse_agent_output
    ...
    (src / "tls.c").write_text("int tls;\n", encoding="utf-8")
    raw = json.dumps({
        "id": "resp-agent-fragments",
        "output": [
            {
                "type": "message",
                "content": [{
                    "type": "output_text",
                    "text": '{"candidate_files": [',
                }],
            },
            {
                "type": "message",
                "content": [{
                    "type": "output_text",
                    "text": (
                        '{"path":"src/tls.c","reason":"responses output stream found source",'
                        '"confidence":"high"}'
                    ),
                }],
            },
            {
                ...
                "content": [{
                    "type": "output_text",
                    "text": '], "candidate_entries": [], "raw_summary": "responses fragments parsed"}',
                }],
            },
        ],
    })
    ...
    assert result.status == "ok"
    assert result.raw_summary == "responses fragments parsed"
    assert result.candidate_files[0].validated is True
    assert result.candidate_files[0].path == "src/tls.c"
...
def test_agent_output_unwraps_json_array_stream_events(tmp_path):
    from app.services.external_agent_discovery import parse_agent_output
    ...
    discovery_payload = json.dumps({
        "candidate_files": [
            {
                "path": "src/tls.c",
                "reason": "valid source path after array stream metadata",
                "confidence": "high",
            }
        ],
        "raw_summary": "array stream parsed",
    })
    raw = json.dumps([
    ...
    assert result.status == "ok"
    assert result.raw_summary == "array stream parsed"
    assert result.candidate_files[0].validated is True
    assert result.candidate_files[0].path == "src/tls.c"


def test_agent_output_prefers_final_stream_result_over_early_empty_schema(tmp_path):
    from app.services.external_agent_discovery import parse_agent_output
    ...
    final_payload = json.dumps({
        "candidate_files": [
            {
                "path": "src/tls.c",
                "reason": "final stream result found source",
                "confidence": "high",
            }
        ],
        "raw_summary": "final stream result parsed",
    })
    raw = json.dumps([
        {"type": "system", "subtype": "init"},
        {
    ...
    assert result.status == "ok"
    assert result.raw_summary == "final stream result parsed"
    assert result.candidate_files[0].validated is True
    assert result.candidate_files[0].path == "src/tls.c"
...
def test_provider_health_auto_discovers_existing_ccr_config_path(tmp_path, monkeypatch):
    from app.config import settings
    from app.services.external_agent_discovery import check_provider_health

    config = tmp_path / "home" / ".claude-code-router" / "config.json"
    config.parent.mkdir(parents=True)
    config.write_text('{"Providers":[]}\n', encoding="utf-8")
    monkeypatch.delenv("CCR_CONFIG_PATH", raising=False)
...
def test_run_provider_nonzero_exit_keeps_ccr_config_hint(tmp_path, monkeypatch):
    from app.services.external_agent_discovery import AgentDiscoveryRequest, run_external_agent_discovery
    ...
    class FakeProc:
        ...
        async def communicate(self, _data):
            return b"", b"ccr failed before routing request\n"
    ...
    results = asyncio.run(run_external_agent_discovery(
        AgentDiscoveryRequest(
            request_id="ccr-config-hint-on-exit",
            repo_path=str(tmp_path),
            analysis_object_text="nvme-tcp-tls",
        ),
        providers=["claude-code"],
...
def test_workspace_path_hint_prioritizes_module_named_source_when_limited(tmp_path):
    ...
    for index in range(8):
        (tls_dir / f"alpha_{index}.c").write_text(
            f"int alpha_{index}(void) {{ return {index}; }}\n",
            encoding="utf-8",
        )
    (tls_dir / "tls.c").write_text("int nvmf_tcp_tls_handshake(void) { return 0; }\n", encoding="utf-8")
    ...
    assert rel_hits[0] == "nvmf_tcp/transport/tls/tls.c"
...
def test_workspace_path_keyword_ranking_handles_line_range_file_location(tmp_path):
    from app.services.workspace_scope_resolver import _path_keyword_repo_hits_blocking
    ...
    hits = _path_keyword_repo_hits_blocking(
        str(tmp_path),
        ["payments/webhook_handler.py:42-50"],
        3,
    )
    rel_hits = [Path(hit).relative_to(tmp_path).as_posix() for hit in hits]
...
def test_workspace_path_keyword_ranking_handles_windows_absolute_file_location(tmp_path):
    from app.services.workspace_scope_resolver import _path_keyword_repo_hits_blocking

    target_dir = tmp_path / "src" / "payments"
    target_dir.mkdir(parents=True)
    target_file = target_dir / "webhook_handler.py"
    ...
    location = r"C:\workspaces\frontend\src\payments\webhook_handler.py:42"
    hits = _path_keyword_repo_hits_blocking(str(tmp_path), [location], 3)
    rel_hits = [Path(hit).relative_to(tmp_path).as_posix() for hit in hits]
...
def test_workspace_path_keyword_ranking_includes_component_source_files(tmp_path):
    from app.services.workspace_scope_resolver import _path_keyword_repo_hits_blocking
    ...
    (target_dir / "PaymentPage.astro").write_text(
        "---\nconst amount = 1\n---\n<div />\n",
        encoding="utf-8",
    )
    ...
    assert rel_hits[0] == "frontend/components/payments/PaymentPage.astro"
...
def test_workspace_exact_symbol_search_prioritizes_ts_arrow_handler_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_routes.ts").write_text(
        "import { paymentHandler } from './z_payment';\n"
        "router.post('/payments', paymentHandler);\n",
        encoding="utf-8",
    )
    (src / "z_payment.ts").write_text(
    ...
    hits = _exact_symbol_repo_hits_blocking(str(tmp_path), "paymentHandler", 4)
    rel_hits = [
        Path(hit).relative_to(tmp_path).as_posix()
        for hit in hits
        if Path(hit).exists()
    ...
    assert rel_hits[0] == "src/z_payment.ts"


def test_workspace_exact_symbol_search_prioritizes_ts_class_field_handler_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_routes.ts").write_text(
        "import { PaymentController } from './z_controller';\n"
        "const controller = new PaymentController();\n"
        "router.post('/payments', controller.paymentHandler);\n",
        encoding="utf-8",
    )
    (src / "z_controller.ts").write_text(
        "export class PaymentController {\n"
        "  public paymentHandler = async (req: Request, res: Response) => {\n"
        "    return res.json({ ok: true });\n"
        "  };\n"
        "}\n",
        encoding="utf-8",
...
def test_workspace_resolver_prioritizes_python_qualified_method_without_tools(
    tmp_path, monkeypatch
):
    src = tmp_path / "src"
    src.mkdir()
    (src / "a_admin.py").write_text(
        "class AdminView:\n"
        "    def post(self, request):\n"
        "        return {'admin': True}\n",
        encoding="utf-8",
    )
    (src / "z_payment.py").write_text(
        "class PaymentView:\n"
        "    def post(self, request):\n"
    ...
    obj = AnalysisObject(
        id="obj_payment_view_post",
        text="PaymentView.post",
        kind="function",
    )
    ...
    first = resolved.candidate_files[0]
    assert Path(first.path).relative_to(tmp_path).as_posix() == "src/z_payment.py"
    assert first.symbol == "post"
    assert first.role == "primary"
    assert first.source == "repo_search"
...
def test_workspace_exact_symbol_search_prioritizes_scala_function_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_controller.scala").write_text(
        "class PaymentController(service: PaymentService) {\n"
        "  def route(request: PaymentRequest): PaymentResult = {\n"
        "    service.processPayment(request)\n"
        "  }\n"
        "}\n",
        encoding="utf-8",
    )
    (src / "z_service.scala").write_text(
        "class PaymentService {\n"
        "  private def processPayment(request: PaymentRequest): PaymentResult = {\n"
        "    PaymentResult()\n"
        "  }\n"
    ...
    assert rel_hits[0] == "src/z_service.scala"


def test_workspace_exact_symbol_search_prioritizes_scala_action_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_routes.scala").write_text(
        "class Routes(controller: PaymentController) {\n"
        "  def route = controller.processPayment\n"
        "}\n",
        encoding="utf-8",
    )
    (src / "z_controller.scala").write_text(
    ...
    assert rel_hits[0] == "src/z_controller.scala"


def test_workspace_exact_symbol_search_prioritizes_ruby_class_method_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_controller.rb").write_text(
        "class PaymentController\n"
        "  def call(request)\n"
    ...
    hits = _exact_symbol_repo_hits_blocking(str(tmp_path), "process_payment", 4)
    rel_hits = [
        Path(hit).relative_to(tmp_path).as_posix()
        for hit in hits
        if Path(hit).exists()
    ...
    assert rel_hits[0] == "src/z_service.rb"


def test_workspace_exact_symbol_search_prioritizes_php_open_tag_function_definition(tmp_path):
    from app.services.workspace_scope_resolver import _exact_symbol_repo_hits_blocking

    src = tmp_path / "src"
    src.mkdir()
    (src / "a_controller.php").write_text(
        "<?php\n"
        "function route_payment($request) {\n"
        "    return process_payment($request);\n"
        "}\n",
        encoding="utf-8",
    )
    (src / "z_service.php").write_text(
        "<?php function process_payment($request) {\n"
        "    return $request;\n"
        "}\n",
        encoding="utf-8",
...
def test_live_gitnexus_fetch_warns_when_repo_is_not_indexed(tmp_path, monkeypatch):
    from app.services import workspace_scope_resolver as resolver
    ...
    assert graph is None
    assert "GitNexus reachable but this repo is not indexed" in warning
    assert "indexed repos: 1" in warning


def test_live_gitnexus_fetch_resolves_value_wrapped_parent_repo(tmp_path, monkeypatch):
    from app.services import workspace_scope_resolver as resolver
                    ...
                    "value": [
                        {
                            "name": "frontend",
                            "path": str(parent),
                            "stats": {"nodes": 9, "edges": 10, "files": 2},
                        }
                    ]
                })
            if path == "/api/graph":
    ...
    graph, warning = asyncio.run(resolver._fetch_live_gitnexus_graph_once(str(child)))

    assert graph == {"nodes": [{"id": "f1"}], "relationships": []}
    assert warning == ""
    assert FakeClient.graph_calls == [{"repo": "frontend", "path": str(parent)}]
...
def test_coverage_local_one_line_function_route_call_adds_external_trigger(
    tmp_path,
    monkeypatch,
):
    ...
    (src / "routes.js").write_text(
        "function normalizeOrder(payload){if(!payload.id)return null;return payload;}\n"
        "app.post('/orders', (req, res) => normalizeOrder(req.body));\n",
        encoding="utf-8",
    )
    ...
    modules = _coverage_modules(
        ...
        "orders,routes,src/routes.js:1-1,normalizeOrder,false,0\n"
    )
...
def test_coverage_local_spring_helper_does_not_inherit_previous_route_decorator(
    tmp_path,
    monkeypatch,
):
    import app.services.coverage_analyzer as coverage_mod
    ...
    (src / "OrderController.java").write_text(
        ...
        "  @PostMapping(\"/{id}\")\n"
        "  Order create(@RequestBody Order dto) { return normalizeOrder(dto); }\n"
        "  Order normalizeOrder(Order input) { return input; }\n"
        "}\n",
        encoding="utf-8",
    )
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "orders,controller,src/OrderController.java:6-6,normalizeOrder,false,0\n"
    )
    ...
    gap = [g for g in design["gaps"] if g.get("function_name") == "normalizeOrder"][0]
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert all(entry.get("chain") != ["normalizeOrder"] for entry in gap["entry_paths"])
    assert gap["entry_paths"][0]["entry_symbol"] == "create"
...
def test_coverage_local_bullmq_worker_keeps_queue_input_hint(tmp_path, monkeypatch):
    ...
    (src / "worker.js").write_text(
        ...
        "  return normalizeOrder(job.data);\n"
        "}\n"
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "orders,worker,src/worker.js:1-3,normalizeOrder,false,0\n"
    )
...
def test_coverage_local_kafka_consumer_keeps_all_topic_input_hints(tmp_path, monkeypatch):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    assert gap["entry_paths"][0]["entry_kind"] == "message"
    assert "orders" in gap["entry_paths"][0]["input_hints"]
    assert "payments" in gap["entry_paths"][0]["input_hints"]
    cases_json = json.dumps(gap["black_box_cases"], ensure_ascii=False)
    assert "orders" in cases_json
    assert "payments" in cases_json


def test_coverage_local_kafka_consumer_keeps_config_topic_input_hint(tmp_path, monkeypatch):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["entry_kind"] == "message"
    assert "config.kafka.topic" in gap["entry_paths"][0]["input_hints"]
    assert "config.kafka.topic" in json.dumps(gap["black_box_cases"], ensure_ascii=False)


def test_registration_config_path_input_hints_strip_method_suffix():
    from app.services.coverage_analyzer import _registration_config_path_input_hints

    assert _registration_config_path_input_hints(
        "const topics = config.kafka.topics.split(',');"
    ) == ["config.kafka.topics"]


def test_coverage_local_rabbitmq_consumer_keeps_queue_input_hint(tmp_path, monkeypatch):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    (src / "consumer.js").write_text(
        ...
        "}\n"
        "function handle(msg) {\n"
        "  return normalizeOrder(JSON.parse(msg.content.toString()));\n"
        "}\n"
...
def test_coverage_local_commander_action_generates_cli_entry(tmp_path, monkeypatch):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    (src / "cli.js").write_text(
        ...
        "  if (Number(value) <= 0) return 8080;\n"
        "  return Number(value);\n"
        "}\n"
        "program.option('--port <port>').action((opts) => normalizePort(opts.port));\n",
        encoding="utf-8",
    )
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "cli,cli,src/cli.js:1-4,normalizePort,false,0\n"
    )
    ...
    gap = [g for g in design["gaps"] if g.get("function_name") == "normalizePort"][0]
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["entry_kind"] == "cli"
    assert gap["entry_paths"][0]["entry_symbol"] == "normalizePort"
    assert gap["entry_paths"][0]["chain"] == ["normalizePort"]
    assert gap["entry_paths"][0]["input_hints"] == ["--port"]
...
def test_coverage_local_python_handler_map_generates_table_entry(tmp_path, monkeypatch):
    ...
    (src / "dispatcher.py").write_text(
        ...
        "    return COMMAND_HANDLERS[command](payload)\n",
        encoding="utf-8",
    )
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "orders,dispatcher,src/dispatcher.py:1-4,normalize_order,false,0\n"
    )
    ...
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["entry_kind"] == "cli"
    assert gap["entry_paths"][0]["entry_symbol"] == "create_order"
    assert gap["entry_paths"][0]["tool"] == "source-table"
    assert gap["entry_paths"][0]["external_trigger"] == "create-order"
    assert "create-order" in json.dumps(gap["black_box_cases"], ensure_ascii=False)
...
def test_file_reader_cross_language_path_builders_keep_argument_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        "var text = Files.readString(Paths.get(baseDir, filename));"
    ) == ["input file", "baseDir", "filename"]
    assert _filesystem_operation_input_hints(
        "var text = Files.readString(Path.of(System.getenv(\"DATA_DIR\"), filename));"
    ) == ["input file", "DATA_DIR", "filename"]
    assert _filesystem_operation_input_hints(
        "payload, err := os.ReadFile(filepath.Join(baseDir, filename))"
    ) == ["input file", "baseDir", "filename"]
    assert _filesystem_operation_input_hints(
        "let payload = std::fs::read_to_string(Path::new(base_dir).join(filename)).unwrap();"
...
def test_c_file_reader_printf_path_buffers_keep_source_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints
    ...
    assert _filesystem_operation_input_hints(
        "char input_path[256];\n"
        'sprintf(input_path, "%s/%s", base_dir, filename);\n'
        'FILE *fp = fopen(input_path, "r");'
    ) == ["input file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        "char *input_path = NULL;\n"
        'asprintf(&input_path, "%s/%s", base_dir, filename);\n'
        'FILE *fp = fopen(input_path, "r");'
    ) == ["input file", "base_dir", "filename"]
...
def test_coverage_local_c_snprintf_reader_keeps_filename_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    (src / "processor.c").write_text(
        "int normalize_payload(const char *payload) {\n"
    ...
    (src / "loader.c").write_text(
        "#include <stdio.h>\n\n"
        "extern int normalize_payload(const char *payload);\n\n"
        "int load_payload(const char *base_dir, const char *filename) {\n"
        "    char path[256];\n"
        "    snprintf(path, sizeof(path), \"%s/%s\", base_dir, filename);\n"
        "    FILE *fp = fopen(path, \"r\");\n"
        "    char buffer[256] = {0};\n"
        "    if (fp) { fread(buffer, 1, sizeof(buffer) - 1, fp); }\n"
        "    return normalize_payload(buffer);\n"
        "}\n",
        encoding="utf-8",
    )
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "files,load,src/processor.c:1-4,normalize_payload,false,0\n"
    )
...
def test_coverage_local_go_filepath_join_reader_keeps_filename_input_hint(
    ...
    (src / "loader.go").write_text(
        ...
        "    \"path/filepath\"\n"
        ")\n\n"
        "func loadPayload(baseDir string, filename string) string {\n"
        "    payload, _ := os.ReadFile(filepath.Join(baseDir, filename))\n"
...
def test_coverage_local_python_pathlib_reader_keeps_generic_path_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_file_reader_destructured_path_aliases_keep_external_field_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        "const { baseDir, filename } = request;\n"
        "const inputPath = path.join(baseDir, filename);\n"
        'const text = fs.readFileSync(inputPath, "utf8");'
    ) == ["input file", "request.baseDir", "request.filename"]
    assert _filesystem_operation_input_hints(
        "const { baseDir: rootDir, filename: name } = request;\n"
        "const inputPath = path.join(rootDir, name);\n"
        'const text = fs.readFileSync(inputPath, "utf8");'
    ) == ["input file", "request.baseDir", "request.filename"]


def test_file_reader_multi_assignment_path_aliases_keep_external_field_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        "base_dir, filename = request.base_dir, request.filename\n"
        "path = os.path.join(base_dir, filename)\n"
        'with open(path, "r") as fh:\n'
        "    text = fh.read()"
    ) == ["input file", "request.base_dir", "request.filename"]
    ...
    assert _filesystem_operation_input_hints(
        "baseDir, filename := request.BaseDir, request.Filename\n"
        "inputPath := filepath.Join(baseDir, filename)\n"
        "payload, _ := os.ReadFile(inputPath)"
    ) == ["input file", "request.BaseDir", "request.Filename"]


def test_file_reader_getter_path_aliases_keep_external_field_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints
    ...
    assert _filesystem_operation_input_hints(
        "base_dir = request.args['base_dir']\n"
...
def test_file_reader_container_aliases_keep_external_field_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints
...
def test_coverage_local_python_multi_assignment_path_reader_keeps_request_field_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    case_text = json.dumps(gap["black_box_cases"], ensure_ascii=False)
    assert "request.base_dir" in case_text
    assert "request.filename" in case_text


def test_coverage_local_python_getter_path_reader_keeps_request_field_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_local_python_container_alias_path_reader_keeps_request_field_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_local_node_destructured_path_reader_keeps_request_field_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_local_python_open_reader_keeps_generic_path_input_hint(
    ...
    modules = _coverage_modules(
        "feature,module,code_location,function,triggered,hit_count\n"
        "files,load,src/load.py:1-2,normalize_text,false,0\n"
    )
...
def test_coverage_local_python_path_object_open_keeps_path_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    ...
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["entry_kind"] == "file"
    assert gap["entry_paths"][0]["input_hints"] == ["input file", "input_path"]
    case_text = json.dumps(gap["black_box_cases"], ensure_ascii=False)
    assert "input_path" in case_text
    assert '"r"' not in case_text


def test_coverage_local_pandas_path_wrapper_keeps_path_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_data_loader_derived_path_expressions_keep_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(input_path.with_suffix('.csv'))"
    ) == ["CSV file", "input_path"]
    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(base_dir / filename)"
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        "frame = pd.read_json(os.path.join(config_dir, config_name))"
    ) == ["JSON file", "config_dir", "config_name"]


def test_data_loader_joinpath_expressions_keep_argument_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(base_dir.joinpath(filename))"
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(base_dir.joinpath(subdir, filename))"
    ) == ["CSV file", "base_dir", "subdir", "filename"]
    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(Path(base_dir).joinpath(filename))"
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        "frame = pd.read_csv(input_path.parent.joinpath(filename))"
...
def test_data_loader_formatted_path_expressions_keep_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv(f"{base_dir}/{filename}")'
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv(f"{os.getenv(\'DATA_DIR\')}/{filename}")'
    ) == ["CSV file", "DATA_DIR", "filename"]
    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv("{}/{}".format(base_dir, filename))'
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv("{path}/{name}".format(path=base_dir, name=filename))'
    ) == ["CSV file", "base_dir", "filename"]


def test_data_loader_percent_formatted_path_expressions_keep_input_hints():
    from app.services.coverage_analyzer import _filesystem_operation_input_hints

    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv("%s/%s" % (base_dir, filename))'
    ) == ["CSV file", "base_dir", "filename"]
    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv("%s/%s" % (os.getenv(\'DATA_DIR\'), filename))'
    ) == ["CSV file", "DATA_DIR", "filename"]
    assert _filesystem_operation_input_hints(
        'frame = pd.read_csv("%s" % input_path)'
    ) == ["CSV file", "input_path"]
...
def test_coverage_local_pandas_derived_path_keeps_path_input_hint(
    ...
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["entry_kind"] == "file"
    assert gap["entry_paths"][0]["input_hints"] == ["CSV file", "input_path"]
    case_text = json.dumps(gap["black_box_cases"], ensure_ascii=False)
    assert "CSV file" in case_text
    assert "input_path" in case_text


def test_coverage_local_pandas_joinpath_keeps_filename_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_local_pandas_fstring_path_keeps_filename_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_local_pandas_percent_format_path_keeps_filename_input_hint(
    tmp_path, monkeypatch
):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
...
def test_coverage_agent_string_chain_with_target_makes_gap_black_box_ready(tmp_path, monkeypatch):
    import app.services.coverage_analyzer as coverage_mod
    from app.services.coverage_analyzer import build_coverage_test_design
    from app.services.external_agent_discovery import AgentCandidateEntry, AgentDiscoveryResult
    ...
    (src / "rpc.c").write_text(
        "void rpc_tls_entry(void) { enqueue_tls_work(); }\n",
        encoding="utf-8",
    )
            ...
            AgentDiscoveryResult(
                ...
                candidate_entries=[
                    AgentCandidateEntry(
                        ...
                        external_trigger="RPC TLS entry",
                        reason="agent returned a text call chain that reaches the gap",
                        validated=True,
                    )
                ],
            )
    ...
    assert gap["black_box_readiness"]["case_type"] == "black_box_ready"
    assert gap["entry_paths"][0]["tool"] == "claude-code"
    assert gap["entry_paths"][0]["chain"] == ["rpc_tls_entry", "internal_gap"]
    card = design["entry_discovery"]["cards"][0]
    candidate = card["candidate_external_entries"][0]
    assert candidate["validation_error"] in (None, "")
...
def test_coverage_agent_entry_chain_missing_target_does_not_generate_black_box_ready(
    tmp_path,
    monkeypatch,
):
    import asyncio
    ...
    (src / "rpc.c").write_text("void rpc_entry(void) { other_gap(); }\n", encoding="utf-8")
            ...
            AgentDiscoveryResult(
                ...
                candidate_entries=[
                    AgentCandidateEntry(
                        ...
                        entry_symbol="rpc_entry",
                        entry_file="src/rpc.c",
                        chain=["rpc_entry", "other_gap"],
                        external_trigger="RPC entry",
                        reason="public RPC handler reaches a different gap",
                        validated=True,
                    )
                ],
            )
    ...
    card = design["entry_discovery"]["cards"][0]
    candidate = card["candidate_external_entries"][0]
    assert candidate["validation_error"] == "chain_missing_target"
