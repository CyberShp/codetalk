...
async def test_workbench_task_run_acceptance_audit_requires_black_box_observability_fields(
    ...
    script_path = tmp_path / "agent_thin_black_box_case.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "(root/'black_box_cases.json').write_text(json.dumps([\n"
        "  {'case_id':'bb_tls_reconnect_timeout',"
        "'title':'TLS reconnect timeout remains externally recoverable',"
        "'steps':['start target','connect initiator','interrupt network','reconnect'],"
        "'expected':['initiator reports timeout','target accepts reconnect']}\n"
        "]), encoding='utf-8')\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(settings, "external_agent_custom_providers", [
    ...
    workflow = {
        "id": "acceptance_thin_black_box_cases_workflow",
        "name": "Acceptance thin black-box cases workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_thin_black_box_cases_workflow",
            "workspace_id": "ws-acceptance-thin-black-box",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["black_box_case_quality:design:black_box_cases.json"]
    assert quality_check["status"] == "invalid"
    assert any("missing_preconditions" in item["reasons"] for item in quality_check["invalid_cases"])
    assert any("missing_observability" in item["reasons"] for item in quality_check["invalid_cases"])
    assert any("missing_diagnostics" in item["reasons"] for item in quality_check["invalid_cases"])


async def test_workbench_task_run_acceptance_audit_requires_black_box_test_directory_mapping(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    script_path = tmp_path / "agent_no_test_dir_black_box_case.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
    ...
    workflow = {
        "id": "acceptance_no_test_dir_black_box_cases_workflow",
        "name": "Acceptance no test dir black-box cases workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_no_test_dir_black_box_cases_workflow",
            "workspace_id": "ws-acceptance-no-test-dir-black-box",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
...
async def test_workbench_task_run_acceptance_audit_rejects_vague_black_box_expected_results(
    ...
    script_path = tmp_path / "agent_vague_black_box_expected.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "(root/'black_box_cases.json').write_text(json.dumps([\n"
        "  {'case_id':'bb_tls_vague_success',"
        "'title':'TLS reconnect should work',"
        "'preconditions':['NVMe-oF target is configured with TLS enabled'],"
        "'steps':['start target','connect initiator','interrupt network','reconnect'],"
        "'expected':['成功','正常','通过'],"
        "'observability':['initiator exit code, target log, connection state metric'],"
    ...
    workflow = {
        "id": "acceptance_vague_black_box_expected_workflow",
        "name": "Acceptance vague black-box expected workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_vague_black_box_expected_workflow",
            "workspace_id": "ws-acceptance-vague-black-box-expected",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
...
async def test_workbench_task_run_acceptance_audit_rejects_duplicate_black_box_cases(
    ...
    script_path.write_text(
        ...
        "'diagnostics':['compare target log timestamps with initiator timeout window'],"
        "'suggested_spdk_test_dir':'test/nvmf'}\n"
        "cases = [dict(case, case_id='bb_tls_timeout_a', title='TLS reconnect timeout recovery'),"
        " dict(case, case_id='bb_tls_timeout_b', title='Reconnect survives TLS timeout')]\n"
    ...
    workflow = {
        "id": "acceptance_duplicate_black_box_cases_workflow",
        "name": "Acceptance duplicate black-box cases workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_duplicate_black_box_cases_workflow",
            "workspace_id": "ws-acceptance-duplicate-black-box",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
...
async def test_workbench_task_run_acceptance_audit_rejects_incomplete_sfmea_risk_findings(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    script_path = tmp_path / "agent_incomplete_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
    ...
    workflow = {
        "id": "acceptance_incomplete_sfmea_workflow",
        "name": "Acceptance incomplete SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_incomplete_sfmea_workflow",
            "workspace_id": "ws-acceptance-incomplete-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    assert quality_check["severity"] == "required"
    assert quality_check["invalid_count"] == 1
    assert any("missing_sfmea_fields" in item["reasons"] for item in quality_check["invalid_findings"])


async def test_workbench_task_run_acceptance_audit_rejects_risk_finding_with_missing_source_file(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    script_path = tmp_path / "agent_missing_source_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "(root/'risk_findings.json').write_text(json.dumps([\n"
        "  {'finding_id':'risk_tls_cleanup','file_path':'src/does_not_exist.c',"
        "'function':'tls_cleanup','summary':'cleanup branch risk',"
        "'failure_mode':'cleanup path leaks TLS context',"
        "'cause':'disconnect during handshake skips release',"
        "'effect':'subsequent connections observe stale TLS state',"
    ...
    workflow = {
        "id": "acceptance_missing_source_sfmea_workflow",
        "name": "Acceptance missing source SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_missing_source_sfmea_workflow",
            "workspace_id": "ws-acceptance-missing-source-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["risk_finding_quality:hunt:risk_findings.json"]
    assert quality_check["status"] == "invalid"
    assert any("source_file_missing" in item["reasons"] for item in quality_check["invalid_findings"])


async def test_workbench_task_run_acceptance_audit_rejects_risk_finding_with_out_of_range_source_line(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    source_path.parent.mkdir()
    source_path.write_text("void tls_cleanup(void) {}\n", encoding="utf-8")
    script_path = tmp_path / "agent_out_of_range_source_line_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "(root/'risk_findings.json').write_text(json.dumps([\n"
        "  {'finding_id':'risk_tls_cleanup','file_path':'src/tls.c','line_start':999,"
        "'function':'tls_cleanup','summary':'cleanup branch risk',"
        "'failure_mode':'cleanup path leaks TLS context',"
        "'cause':'disconnect during handshake skips release',"
        "'effect':'subsequent connections observe stale TLS state',"
        ...
        "'severity_score':8,'occurrence_score':4,'detection_score':5,'rpn':160,"
        "'mitigation':'add disconnect/reconnect black-box test and monitor logs',"
        "'score_explanation':'severity=8 because reconnect can fail; occurrence=4 on disconnect; "
        "detection=5 via logs and connection metrics'}\n"
        "]), encoding='utf-8')\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(settings, "external_agent_custom_providers", [
    ...
    workflow = {
        "id": "acceptance_out_of_range_source_line_sfmea_workflow",
        "name": "Acceptance out-of-range source line SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_out_of_range_source_line_sfmea_workflow",
            "workspace_id": "ws-acceptance-out-of-range-source-line-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["risk_finding_quality:hunt:risk_findings.json"]
    assert quality_check["status"] == "invalid"
    assert any("source_line_out_of_range" in item["reasons"] for item in quality_check["invalid_findings"])


async def test_workbench_task_run_acceptance_audit_requires_risk_finding_source_file(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    script_path = tmp_path / "agent_no_source_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "(root/'risk_findings.json').write_text(json.dumps([\n"
        "  {'finding_id':'risk_tls_cleanup',"
        "'function':'tls_cleanup','summary':'cleanup branch risk',"
        "'failure_mode':'cleanup path leaks TLS context',"
        "'cause':'disconnect during handshake skips release',"
        "'effect':'subsequent connections observe stale TLS state',"
    ...
    workflow = {
        "id": "acceptance_no_source_sfmea_workflow",
        "name": "Acceptance no source SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_no_source_sfmea_workflow",
            "workspace_id": "ws-acceptance-no-source-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["risk_finding_quality:hunt:risk_findings.json"]
    assert quality_check["status"] == "invalid"
    assert any("source_file_required" in item["reasons"] for item in quality_check["invalid_findings"])


async def test_workbench_task_run_acceptance_audit_rejects_duplicate_sfmea_risk_findings(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    source_path.parent.mkdir()
    source_path.write_text("void tls_cleanup(void) {}\n", encoding="utf-8")
    script_path = tmp_path / "agent_duplicate_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        "finding = {"
        "'file_path':'src/tls.c','function':'tls_cleanup',"
        "'failure_mode':'cleanup path leaks TLS context',"
        "'cause':'disconnect during handshake skips release',"
        "'effect':'subsequent connections observe stale TLS state',"
        "'detection':'watch target logs and connection state after disconnect',"
        ...
        "'severity_score':8,'occurrence_score':4,'detection_score':5,'rpn':160,"
        "'mitigation':'add disconnect/reconnect black-box test and monitor logs'}\n"
        "findings = [dict(finding, finding_id='risk_tls_cleanup_a', summary='cleanup branch risk'),"
        " dict(finding, finding_id='risk_tls_cleanup_b', summary='TLS cleanup duplicate risk')]\n"
        "(root/'risk_findings.json').write_text(json.dumps(findings), encoding='utf-8')\n",
        encoding="utf-8",
    )
    monkeypatch.setattr(settings, "external_agent_custom_providers", [
        {"id": "local-python", "command": f"python {script_path}"}
    ...
    workflow = {
        "id": "acceptance_duplicate_sfmea_workflow",
        "name": "Acceptance duplicate SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_duplicate_sfmea_workflow",
            "workspace_id": "ws-acceptance-duplicate-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
...
async def test_workbench_task_run_acceptance_audit_requires_sfmea_score_explanation(
    ...
    source_path.parent.mkdir()
    source_path.write_text("void tls_cleanup(void) {}\n", encoding="utf-8")
    script_path = tmp_path / "agent_no_score_explanation_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
    ...
    workflow = {
        "id": "acceptance_no_score_explanation_sfmea_workflow",
        "name": "Acceptance no score explanation SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_no_score_explanation_sfmea_workflow",
            "workspace_id": "ws-acceptance-no-score-explanation-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["risk_finding_quality:hunt:risk_findings.json"]
    assert quality_check["status"] == "invalid"
    assert any("missing_score_explanation" in item["reasons"] for item in quality_check["invalid_findings"])


async def test_workbench_task_run_acceptance_audit_rejects_non_actionable_sfmea_mitigation(
    workbench_client,
    tmp_path,
    monkeypatch,
):
    ...
    source_path.parent.mkdir()
    source_path.write_text("void tls_cleanup(void) {}\n", encoding="utf-8")
    script_path = tmp_path / "agent_weak_mitigation_sfmea.py"
    script_path.write_text(
        "import json, os, pathlib, sys\n"
        "json.load(sys.stdin)\n"
        "root=pathlib.Path(os.environ['CODETALK_AGENT_ARTIFACT_DIR'])\n"
        ...
        "'severity':'high',"
        "'severity_score':8,'occurrence_score':4,'detection_score':5,'rpn':160,"
        "'mitigation':'improve robustness and review implementation quality',"
        "'score_explanation':'severity=8 because leaked TLS state affects reconnect; "
        "occurrence=4 from disconnect branch frequency; detection=5 via target logs and connection metrics'}\n"
        "]), encoding='utf-8')\n",
        encoding="utf-8",
    ...
    workflow = {
        "id": "acceptance_weak_mitigation_sfmea_workflow",
        "name": "Acceptance weak mitigation SFMEA workflow",
        "version": 1,
        "inputs": [{"id": "module", "type": "free_text"}],
        "steps": [
            {
    ...
    prepared = await workbench_client.post(
        "/api/workbench/task-runs/prepare",
        json={
            "workflow_id": "acceptance_weak_mitigation_sfmea_workflow",
            "workspace_id": "ws-acceptance-weak-mitigation-sfmea",
            "repo_path": str(tmp_path),
            "inputs": {"module": "nvme-tcp-tls"},
        },
    )
    ...
    quality_check = checks["risk_finding_quality:hunt:risk_findings.json"]
    assert quality_check["status"] == "invalid"
    assert any("non_actionable_mitigation" in item["reasons"] for item in quality_check["invalid_findings"])
