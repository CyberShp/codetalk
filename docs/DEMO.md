# CodeTalks 产品演示文档

> **代码分析编排平台** — 一站式代码运行时风险分析与知识管理
>
> 版本 1.1 · 2026-04-27

---

## 产品定位

CodeTalks 是一个**代码分析编排与可视化平台**。它不重新发明轮子，而是将代码图谱、覆盖率分析、AI 线程和智能体工作流统一编排，通过一个现代化 Web 界面呈现分析结果。

**核心价值**：导入代码仓库 → 一键触发分析 → 多维度可视化运行时风险

**技术架构**：
```
Browser → Next.js 前端 → FastAPI 后端 → 编排与分析能力
                                          ├── GitNexus（知识图谱）
                                          ├── Coverage Analyzer（覆盖率/黑盒用例）
                                          ├── AI Thread（源码证据问答）
                                          └── Workbench（智能体工作流）
```

---

## 核心功能演示

### 一、仪表盘 — 全局态势感知

![仪表盘](demo-images/01_dashboard.png)

**能力说明**：
- 四维统计卡片：项目总数、运行中任务、已完成任务、工具健康状态（4/4 全在线）
- 最近活动流：实时展示最近 5 条分析任务及其状态
- 运行中任务面板：带进度条的实时任务监控，5 秒自动刷新
- 顶部全局搜索：跨项目、跨任务快速检索

**应用场景**：团队负责人每日登录后第一眼看到整体分析状况，快速判断是否有异常。

---

### 二、风险总览 — 从哪里开始

![风险总览](demo-images/05_analysis_overview.png)

**能力说明**：
- **风险矩阵仪表盘**：首屏展示全部函数的风险排名，四项统计卡片带趋势箭头（与上次分析对比）
- **热点函数高亮**：最高风险函数在顶部醒目展示，一键跳转到深度分析
- **可排序/可过滤/可分页**：按风险评分、控制结构数、密度排序，按 HIGH/MED/LOW 过滤
- **CSV 导出**：一键导出完整风险矩阵表，UTF-8 编码兼容 Excel
- **快照持久化**：分析结果自动存库，趋势标注基于与上次快照的对比
- **一键重建分析**：右上角按钮触发 CPG 重建 + 风险矩阵自动刷新

**应用场景**：技术评审会议上，首屏看到"哪些函数最危险"，直接点击进入深度分析。

---

### 三、深度分析 — 全维度函数体检

![深度分析 kill_iscsid](demo-images/06_branch_kill_iscsid.png)

**能力说明**（以 `kill_iscsid` 函数为例）：
- **方法风险摘要卡**：查询后首先展示控制结构数、代码行数、复杂度密度、风险评分，卡片颜色按风险等级编码
- **控制流图 (CFG)**：函数控制流可视化，支持缩放平移
- **调用上下文**：`main` 函数在 `L3904` 调用 `kill_iscsid(killiscsid, timeout)`，调用者分支展示 IF / WHILE / SWITCH / GOTO / BREAK 等完整控制流路径
- **被调用影响**：`kill_iscsid` 调用了 `log_error`、`iscsid_exec_req`、`iscsi_err_print_msg` 三个下游函数
- **模块影响面**（跨工具融合）：Joern 调用者文件 + GitNexus 模块级依赖链，评估修改该函数的影响半径
- **控制流分支**：两处关键分支 — `priority != 0` 和 `rc != 0` 的异常处理路径
- 每个分支标注文件路径 + 行号（如 `usr/iscsiadm.c:303`）

**应用场景**：从风险总览点击函数 → 一页看到 CFG + 风险评分 + 调用链 + 影响面 + 分支 + 边界值，无需切换 tab。

---

### 四、数据传播追踪 — 追踪异常数据流向

![数据传播追踪](demo-images/07_taint.png)

**能力说明**：
- **4 种预设检测模式**：数值溢出、空指针解引用、边界越界、资源泄漏
- 自定义 Source / Sink 模式：灵活定义数据来源和风险操作点
- 可视化传播链：Source（绿色）→ 中间节点 → Sink（红色），每个节点标注代码位置
- 基于 Joern CPG 引擎，进行跨函数的深度数据流分析

**应用场景**：开发团队定向追查某个外部输入在经过多层函数调用后，是否可能触发数值溢出、数组越界或空指针解引用等运行时异常。

---

### 五、知识图谱 — 代码结构可视化

![神经图谱 Function 筛选](demo-images/04_graph_function.png)

**能力说明**：
- **6221 个节点**的大规模代码知识图谱：
  - Function 1960 / Struct 1033 / File 282 / Process 300 / Macro 2375 / Enum 93 / Class 3 / Community 175
- 按类型筛选：点击 Function 按钮，所有函数按调用树状图排列，上下文关系一目了然
- 三种搜索模式：Hybrid / BM25 / Semantic

![神经图谱 kill_iscsid 详情](demo-images/04_graph_kill_iscsid.png)

- 点击 `kill_iscsid` 节点展开详情面板：文件位置 `usr/iscsiadm.c:288`，下游调用 `iscsid_exec_req`、`iscsi_err_print_m...`
- **「在 CHAT 中追问」** 按钮：针对选中节点直接向 AI 提问，结合源码上下文深入分析

**应用场景**：新人 onboarding 理解项目架构，或跨团队 code review 时快速掌握函数调用关系。

---

### 六、工作区报告 — 自动化项目梳理

![AI 文档生成](demo-images/03_wiki.png)

**能力说明**：
- 基于已选择的工作区源码、代码图谱和用户输入生成结构化报告
- 报告按模块、流程、风险、测试建议组织，保留源码证据和文件位置
- 文档内含代码引用链接（如 `usr/meson.build`、`Makefile`），点击直接跳转到源文件
- AI 线程可继续追问测试建议、SFMEA 和黑盒测试用例
- 支持导出 Markdown / JSON 产物

**应用场景**：接手老项目时，不再需要花数天阅读代码，先由工作区报告梳理结构，再在 AI 线程中追问源码证据。

---

### 七、全屏 AI 问答 — SFMEA 失效模式分析

![AI SFMEA kill_iscsid](demo-images/12_ai_sfmea_kill_iscsid.png)

**能力说明**（以 `kill_iscsid` 函数为例）：
- 全屏 AI 聊天界面，优先读取当前工作区源码、上传材料和任务产物
- 输入"请针对 kill_iscsid 生成 SFMEA"，AI 结合工作区源码证据生成失效模式分析
- 每条 SFMEA 包含：失效模式描述、失效原因列表、严重度评分（1-10）、具体检测方法
- AI 回答必须引用可追溯的源码文件、函数名、常量名和任务产物

**应用场景**：安全审计或 FMEA 评审时，快速生成特定函数的失效模式分析表，节省人工梳理时间。

---

## 次核心功能演示

### 八、测试点生成 — AI 辅助风险测试

![测试点生成 kill_iscsid](demo-images/08_testpoints_kill_iscsid.png)

**能力说明**（以 `kill_iscsid` 函数为例）：
- **联合分析引擎**：Joern CPG 控制流分析 + GitNexus 调用链解析 + AI 生成
- 指定目标函数 `kill_iscsid`，生成 4 个高质量测试点
- 每个测试点包含：风险等级（HIGH/MEDIUM）、分类（BOUNDARY/ERROR_HANDLING）、输入条件、预期行为、风险场景、边界值
- 示例：`{"priority": [-1, 0, 1]}` — 精确到参数级别的边界值建议
- 支持导出为 Markdown / JSON 格式

**应用场景**：QA 团队根据生成的测试点编写边界条件和异常分支测试用例，覆盖优先级校验、IPC 通信超时、进程信号发送失败等运行时风险场景。

---

### 九、资产管理 — 项目与仓库组织

![资产管理](demo-images/02_assets.png)

**能力说明**：
- 左右分栏：左侧项目树 → 右侧仓库列表
- 支持 Git URL 和本地路径两种代码导入方式
- 一键同步（git pull），一键分析
- 仓库表格展示：来源、URI、分支、最近索引时间
- 支持项目/仓库级别的删除（含确认弹窗）

**应用场景**：团队按项目组织多个代码仓库，统一管理分析任务。

---

### 十、工具监控 — 引擎健康可视化

![工具监控](demo-images/09_tools.png)

**能力说明**：
- 5 个分析工具卡片式展示，每个标注在线/离线/忙碌状态
- 每个工具列出具体能力标签（如 knowledge_graph、taint_analysis、risk_scan）
- 平台核心理念展示："所有分析由工具本身执行 — CodeTalks 仅管理执行流程并展示结果"
- 当前 4/4 核心工具在线，CodeCompass 即将推出

**应用场景**：运维人员实时监控工具健康，快速定位离线工具。

---

### 十一、LLM 配置 — 灵活对接内网 AI

![系统设置](demo-images/10_settings.png)

**能力说明**：
- AI 摘要功能开关（可按需启停）
- 多 LLM 配置管理：支持 OpenAI / Ollama / Google / OpenRouter / AWS Bedrock / 自定义
- 每个配置支持独立测试、设为默认、编辑、删除
- 代理模式切换：系统代理 / 直连（适配内网环境）
- API Key 加密存储（Fernet 加密）

**应用场景**：内网部署时对接本地 Ollama 或内网 LLM 代理，无需外网连接。

---

### 十二、任务管理 — 分析全生命周期

![任务管理](demo-images/11_tasks.png)

**能力说明**：
- 5 种状态过滤：全部 / 运行中 / 已完成 / 失败 / 等待中
- 任务表格：ID、仓库、类型、工具（彩色标签）、进度条、状态、操作
- 支持停止运行中任务、删除历史任务
- 全局「新建分析」快捷入口

**应用场景**：分析工程师管理批量扫描任务，追踪进度和状态。

---

## 技术亮点

| 维度 | 说明 |
|------|------|
| **纯编排架构** | CodeTalks 零分析逻辑，100% 依赖外部工具 — 移除任何工具容器后系统报连接错误而非静默降级 |
| **工具可插拔** | 每个分析引擎独立容器化，可按需启停，互不影响 |
| **跨工具融合** | Joern 调用链 + GitNexus 依赖图叠加，自动计算影响半径 |
| **分析持久化** | 风险矩阵快照自动存库，趋势对比基于真实数据变化 |
| **容器故障自愈** | 验证 166/166 用例通过：工具容器 stop → 优雅降级 → restart → 数据完整恢复 |
| **零内存泄漏** | 压力测试（10 轮连续扫描 + 30 分钟 soak）堆内存零增长 |
| **深色科技主题** | Kinetic Shadow 设计语言，毛玻璃效果面板，三色强调体系 |

---

## 部署方式

| 方式 | 说明 |
|------|------|
| **Docker Compose** | 本地开发/演示，一键 `docker compose up` |

---

## 后续规划

- CodeCompass 集成（C/C++ 调用图 + 指针分析）
- 多人协作与权限管理
- 分析报告导出（PDF/HTML）
- CI/CD Pipeline 集成（GitLab CI / Jenkins）
- 快照历史对比抽屉（两次分析结果 diff 可视化）

---

*CodeTalks — 让代码运行时风险分析从「工具散落」走向「编排一体」*
