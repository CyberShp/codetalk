...
CodeTalks 是一个**代码分析编排与可视化平台**。它不重新发明轮子，而是将 5 个业界领先的开源分析引擎统一编排，通过一个现代化 Web 界面呈现分析结果。

**核心价值**：导入代码仓库 → 一键触发分析 → 多维度可视化运行时风险

**技术架构**：
```
Browser → Next.js 前端 → FastAPI 后端 → 5 个分析引擎
                                          ├── DeepWiki（AI 文档生成）
                                          ├── GitNexus（知识图谱）
                                          ├── Joern（CPG 运行时风险分析）
                                          ├── Semgrep（规则匹配扫描）
...
**能力说明**：
- 四维统计卡片：项目总数、运行中任务、已完成任务、工具健康状态（5/5 全在线）
...
- **双引擎联动**：Joern CPG（控制流/数据流分析）+ Semgrep（规则匹配）同时在线
- **一键 Re-scan**：右上角按钮触发增量扫描，平均 17 秒完成

**应用场景**：技术评审会议上，快速了解项目运行时风险面貌。50 个风险点，包含异常分支、边界值和数值翻转等运行时隐患。

---

### 三、风险点详情 — 精准定位运行时隐患

![风险点列表](demo-images/06_findings.png)

**能力说明**：
- 50 条风险点，每条标注风险等级（High Risk / Medium Risk / Low Risk）
- 规则 ID 可追溯（如 `insecure-use-strtok-fn` → 运行时可能导致字符串截断异常）
- 展开后显示：
  - 精确代码位置（文件路径 + 行号）
  - CWE 编号（如 CWE-676: 使用可能在极端输入下异常的函数）
  - 运行时影响说明与修复建议
- 左侧过滤器支持按风险等级、类别筛选

**应用场景**：开发人员逐一排查运行时风险点，了解异常分支和边界条件下的行为隐患。
...
**能力说明**：
- **Joint Analysis Engine**：联合 Joern + Semgrep 分析结果 + AI 生成
- 可指定目标函数，也可全量生成
...
| **工具可插拔** | 每个分析引擎独立容器化，可按需启停，互不影响（Joern 负责控制流/数据流，Semgrep 负责规则匹配） |
