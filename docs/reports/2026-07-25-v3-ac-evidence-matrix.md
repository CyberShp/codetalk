...
| AC-SEC-005 | partial | `scripts/generate-offline-sbom.py` creates a local-only manifest plus a fail-closed `license-review.json` from backend requirements, frontend lockfile and installed metadata. The 2026-07-27 run recorded 692 components, 623 resolved local licenses, 69 explicit `UNKNOWN` items and no unresolved backend dependency. Human license approval and approved offline-bundle evidence remain required. |
