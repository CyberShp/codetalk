...
| AC-SEC-005 | partial | `scripts/generate-offline-sbom.py` creates a local-only manifest plus a fail-closed `license-review.json` from backend requirements, frontend lockfile and installed metadata. The 2026-07-27 run recorded 692 components, 623 resolved local licenses, 69 explicit `UNKNOWN` items and no unresolved backend dependency. `generate-offline-bundle-manifest.py` now also records SHA256/name/version/license/platform for the 64 local SDK POC artifacts, correctly leaving Claude SDK's “SEE LICENSE IN README.md” as an explicit unknown. Human license approval and a complete platform-specific CodeTalk offline-bundle install evidence remain required. |
