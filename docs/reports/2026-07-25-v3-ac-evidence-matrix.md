...
| AC-SEC-006 | partial | xyflow is locally bundled in the app; a 2026-07-27 Chromium reload of `/workflows` recorded no non-local browser request and no console error (`/Volumes/Media/codetalk-e2e-artifacts/v3-designer-local-assets-20260727/`). Final deployment capture must still prove the packaged designer makes no CDN/update/plugin/telemetry request. |
