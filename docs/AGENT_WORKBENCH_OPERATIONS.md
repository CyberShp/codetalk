...
`task_acceptance_audit.json` is generated by
`POST /api/workbench/task-runs/<task_run_id>/acceptance-audit`. It checks the
required preparation artifacts, provider readiness, Agent run envelopes,
execution artifacts, required Agent outputs, rerun plans for incomplete runs,
and the artifact manifest itself. Use it as the local acceptance gate before
calling an intranet task run healthy.
