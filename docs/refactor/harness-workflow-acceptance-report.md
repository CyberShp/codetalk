...
证据不能替代管理员拥有的 PCAP/网关日志，也不能把 loopback Codex-compatible CLI
adapter 说成实际安装的 Codex 经企业网关运行。实际 Codex 仍因没有管理员批准的 Base
URL、代理、CA 或 egress gateway 而未调用。首位全新独立 reviewer 已给出 `REJECT`，
...
门禁现已通过；总体仍因 AC-10 管理员 PCAP/网关负向证据和 AC-12 实际安装 Codex 经批准
内网网关的真实运行而保持 `Blocked`，不得据此宣称可发布。
...
| 2026-07-29 第八轮独立准入审核 | `APPROVE`，P0=0、P1=0、P2=0、P3=0；reviewer 独立验证 changed-area `276 passed`、前端静态 `13 passed`、TypeScript 和 `git diff --check`。 | reviewer 门禁通过；reviewer 同时确认 AC-10/AC-12 仍准确标为外部 `Blocked`，未把它们误写为已验收。 |
...
| AC-12 通用与正式存储测试真实 E2E | Blocked | Phase 7 真实 Chrome 已通过 Builtin fixture、loopback Codex-compatible CLI adapter 和正式存储设计显式治理三条流程。 | 实际安装的 Codex 尚无管理员批准网关，未运行；因此不能把适配器/fixture 证据升级为完整内网 Provider 验收。 |
...
| 实际安装的 Codex：经批准内网网关运行同一流程 | Blocked | 没有管理员批准的 Base URL、代理、CA 或 egress gateway；未调用 public Codex。 |
...
下列任一项未关闭时，整体状态保持 `Blocked`：

1. AC-10 的管理员 PCAP、网关日志和负向出口证据；
2. AC-12 中实际安装 Codex 经管理员批准内网网关的真实运行；
3. 一位全新、未参与 Phase 7 实现的只读 reviewer 明确给出 `APPROVE`，P0=0、P1=0；
...
当前最新 reviewer verdict 为 `APPROVE`，P0/P1/P2/P3 均为零，reviewer 准入门禁已关闭。
但 AC-10 与 AC-12 仍是 `Blocked` 发布条件，因此不得宣布 Phase 7 发布验收完成，也不得
把本地适配器/fixture 证据表述为实际安装 Codex 经管理员批准网关的运行。
